...
# edited by Marco Scandaletti - www.scanda.it
# variables were initialized with sample values, replace them with values from your environment
# cpu, ram and storage are recommended values

#                  script for site B


# vCenter Server used to deploy vSphere Lab
$VIServer = "vcenter.scanda.lab"
$VIUsername = "administrator@vsphere.local"
$VIPassword = "VMware1!VMware1!"

# Full Path to both the Nested ESXi 7.0 VA, Extracted VCSA 7.0 ISO & NSX-T OVAs
$NestedESXiApplianceOVA = "/mnt/OVA/Nested_ESXi7.0u3c_Appliance_Template_v1.ova"
$VCSAInstallerPath = "/mnt/OVA/VMware-VCSA-all-7.0.3-19234570"   # <--- Extracted VCSA 7.0 ISO
$NSXTManagerOVA = "/mnt/OVA/nsx-unified-appliance-3.1.3.7.0.19380482.ova"
$NSXTEdgeOVA = "/mnt/OVA/nsx-edge-3.1.3.7.0.19380486.ova"
...
$VCSADisplayName = "vcb"
$VCSAIPAddress = "192.168.40.10"
$VCSAHostname = "vcb.scanda.local" #Change to IP if you don't have valid DNS
$VCSAPrefix = "24"
...
$VMCluster = "Cluster"
$VMNetwork = "Mgmt-SiteB"
$VMDatastore = "Datastore"
$VMNetmask = "255.255.255.0"
$VMGateway = "192.168.40.254"
$VMDNS = "192.168.20.199"
$VMNTP = "pool.ntp.org"
$VMPassword = "VMware1!VMware1!"
$VMDomain = "scanda.local"
$VMSyslog = "192.168.20.200"
$VMFolder = "siteB"
...
$NewVCDatacenterName = "siteB-Datacenter"
$NewVCVSANClusterName = "siteB-Cluster"
$NewVCVDSName = "siteB-VDS"
$NewVCDVPGName = "DVPG-Management-siteB"

# Tanzu Configuration
$StoragePolicyName = "notused"
$StoragePolicyTagCategory = "notused"
$StoragePolicyTagName = "notused"
$DevOpsUsername = "devops"
$DevOpsPassword = "VMware1!VMware1!"

# NSX-T Configuration
$NSXLicenseKey = "XXXXX-XXXXX-XXXXX-XXXXX-XXXXX"  # use a valid NSX-T License
...
$NSXVTEPNetwork = "Overlay-SiteB" # This portgroup needs be created before running script

# Transport Node Profile
$TransportNodeProfileName = "siteB-Host-Transport-Node-Profile"

# TEP IP Pool
$TunnelEndpointName = "TEP-IP-Pool-siteB"
$TunnelEndpointDescription = "Tunnel Endpoint for Transport Nodes"
$TunnelEndpointIPRangeStart = "192.168.45.50"
$TunnelEndpointIPRangeEnd = "192.168.45.70"
$TunnelEndpointCIDR = "192.168.45.0/24"
$TunnelEndpointGateway = "192.168.45.254"

# Transport Zones
$OverlayTransportZoneName = "TZ-Overlay-siteB"
$OverlayTransportZoneHostSwitchName = "nsxswitch"
$VlanTransportZoneName = "TZ-VLAN-siteB"
$VlanTransportZoneNameHostSwitchName = "edgeswitch"

# Network Segment
$NetworkSegmentName = "siteB-Segment"
$NetworkSegmentVlan = "0"

# T0 Gateway
$T0GatewayName = "siteB-T0-Gateway"
$T0GatewayInterfaceAddress = "10.40.0.109" # should be a routable address
...
$T0GatewayInterfaceStaticRouteAddress = "10.40.0.254"
...
# Edge Cluster
$EdgeClusterName = "Edge-Cluster-siteB"

# NSX-T Manager Configurations
$NSXTMgrDeploymentSize = "small"
$NSXTMgrvCPU = "6" #override default size
$NSXTMgrvMEM = "24" #override default size
$NSXTMgrDisplayName = "nsxlb1"
$NSXTMgrHostname = "nsxlb1.scanda.local"
$NSXTMgrIPAddress = "192.168.40.51"

# NSX-T Global Manager Configurations
$NSXTGMgrDeploymentSize = "small"
$NSXTGMgrvCPU = "6" #override default size
$NSXTGMgrvMEM = "24" #override default size
$NSXTGMgrDisplayName = "nsxgb1"
$NSXTGMgrHostname = "nsxgb1.scanda.local"
$NSXTGMgrIPAddress = "192.168.40.61"
...
$debug = $true
$verboseLogFile = "vsphere-with-nsxt-siteB.log"
$random_string = -join ((65..90) + (97..122) | Get-Random -Count 8 | % {[char]$_})
$VAppName = "SiteB-Nested-vSphere-with-NSX-T-$random_string"
...
$deployNSXLocalManager = 1
$deployNSXGlobalManager = 1
$deployNSXEdge = 1
...
if($preCheck -eq 1) {
    ...
    if(!(Test-Path $NSXTManagerOVA) -and $deployNSXLocalManager -eq 1) {
...
if($confirmDeployment -eq 1) {
    ...
    if($deployNSXLocalManager -eq 1) {
        Write-Host -NoNewline -ForegroundColor Green "NSX-T Manager Image Path: "
        Write-Host -ForegroundColor White $NSXTManagerOVA
    }
    if($deployNSXGlobalManager -eq 1) {
        Write-Host -NoNewline -ForegroundColor Green "NSX-T Global Manager Image Path: "
        Write-Host -ForegroundColor White $NSXTManagerOVA
    }
    ...
    if($deployNSXLocalManager -eq 1 -or $deployNSXEdge -eq 1) {
    ...
    if($deployNSXGlobalManager -eq 1) {
        Write-Host -NoNewline -ForegroundColor Green "NSX Global Manager Hostname: "
        Write-Host -ForegroundColor White $NSXTGMgrHostname
        Write-Host -NoNewline -ForegroundColor Green "NSX Global Manager IP Address: "
        Write-Host -ForegroundColor White $NSXTGMgrIPAddress
        Write-Host -NoNewline -ForegroundColor Green "Netmask: "
        Write-Host -ForegroundColor White $VMNetmask
        Write-Host -NoNewline -ForegroundColor Green "Gateway: "
        Write-Host -ForegroundColor White $VMGateway
    ...
    if($deployNSXLocalManager -eq 1 -or $deployNSXGlobalManager -eq 1 -or $deployNSXEdge -eq 1) {
        if($deployNSXLocalManager -eq 1) {
        ...
        if($deployNSXGlobalManager -eq 1) {
            $nsxGManagerTotalCPU += [int]$NSXTGMgrvCPU
            $nsxGManagerTotalMemory += [int]$NSXTGMgrvMEM
            $nsxGManagerTotalStorage += [int]$nsxStorageMap["manager"]

            Write-Host -NoNewline -ForegroundColor Green "NSX-Global-UA   VM CPU: "
            Write-Host -NoNewline -ForegroundColor White $nsxGManagerTotalCPU
            Write-Host -NoNewline -ForegroundColor Green " NSX-Global-UA   VM Memory: "
            Write-Host -NoNewline -ForegroundColor White $nsxGManagerTotalMemory "GB "
            Write-Host -NoNewline -ForegroundColor Green " NSX-Global-UA   VM Storage: "
            Write-Host -ForegroundColor White $nsxGManagerTotalStorage "GB"
        }
...
if( $deployNestedESXiVMs -eq 1 -or $deployVCSA -eq 1 -or $deployNSXLocalManager -eq 1 -or $deployNSXGlobalManagerManager -eq 1 -or $deployNSXEdge -eq 1) {
...
if($deployNSXGlobalManager -eq 1) {
    # Deploy NSX Global Manager
    $nsxMgrOvfConfig = Get-OvfConfiguration $NSXTManagerOVA
    $nsxMgrOvfConfig.DeploymentOption.Value = $NSXTGMgrDeploymentSize
    $nsxMgrOvfConfig.NetworkMapping.Network_1.value = $VMNetwork

    $nsxMgrOvfConfig.Common.nsx_role.Value = "NSX Global Manager"
    $nsxMgrOvfConfig.Common.nsx_hostname.Value = $NSXTGMgrHostname
    $nsxMgrOvfConfig.Common.nsx_ip_0.Value = $NSXTGMgrIPAddress
    $nsxMgrOvfConfig.Common.nsx_netmask_0.Value = $VMNetmask
    $nsxMgrOvfConfig.Common.nsx_gateway_0.Value = $VMGateway
    $nsxMgrOvfConfig.Common.nsx_dns1_0.Value = $VMDNS
    $nsxMgrOvfConfig.Common.nsx_domain_0.Value = $VMDomain
    ...
    My-Logger "Deploying NSX Global Manager VM $NSXTGMgrDisplayName ..."
    $nsxgmgr_vm = Import-VApp -Source $NSXTManagerOVA -OvfConfiguration $nsxMgrOvfConfig -Name $NSXTGMgrDisplayName -Location $cluster -VMHost $vmhost -Datastore $datastore -DiskStorageFormat thin

    My-Logger "Updating vCPU Count to $NSXTGMgrvCPU & vMEM to $NSXTGMgrvMEM GB ..."
    Set-VM -Server $viConnection -VM $nsxgmgr_vm -NumCpu $NSXTGMgrvCPU -MemoryGB $NSXTGMgrvMEM -Confirm:$false | Out-File -Append -LiteralPath $verboseLogFile

    My-Logger "Disabling vCPU Reservation ..."
    Get-VM -Server $viConnection -Name $nsxgmgr_vm | Get-VMResourceConfiguration | Set-VMResourceConfiguration -CpuReservationMhz 0 | Out-File -Append -LiteralPath $verboseLogFile

    My-Logger "Powering On $NSXTMgrDisplayName ..."
    $nsxgmgr_vm | Start-Vm -RunAsync | Out-Null
}
...
if($moveVMsIntovApp -eq 1) {
    ...
    if($deployNSXGlobalManager -eq 1) {
        $nsxGMgrVM = Get-VM -Name $NSXTGMgrDisplayName -Server $viConnection
        My-Logger "Moving $NSXTGMgrDisplayName into $VAppName vApp ..."
        Move-VM -VM $nsxGMgrVM -Server $viConnection -Destination $VApp -Confirm:$false | Out-File -Append -LiteralPath $verboseLogFile
    }
...
if( $deployNestedESXiVMs -eq 1 -or $deployVCSA -eq 1 -or $deployNSXLocalManager -eq 1 -or $deployNSXGlobalManager -eq 1 -or $deployNSXEdge -eq 1) {
...
if($postDeployNSXConfig -eq 1) {
    ...
    $runT0Gateway=$false #$true
    $runT0StaticRoute=$false #$true
